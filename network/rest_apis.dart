...
Future<List<PostModel>> getPost({int page = 1, int? userId, int? groupId, required String type}) async {
  print('id#######'+appStore.loginUserId.toInt().toString()+'perpage'+ PER_PAGE.toString()+'page'+page.toString()+'grooup id'+groupId.toString());
...
Future<List<WpPostResponse>> getBlogList({int? page}) async {
  Iterable it = await handleResponse(await buildHttpResponse('${APIEndPoint.wpPost}?_embed&page=$page&per_page=$PER_PAGE&categories_exclude=185,193,194', method: HttpMethod.GET));
...
Future<List<ProductListModel>> getProductsList({int page = 1, int? categoryId, String? orderBy, String? searchText}) async {
  ...
  if (categoryId != null) {
    it = await handleResponse(await buildHttpResponse('${APIEndPoint.productsList}?category=$categoryId&exclude=10267,10271&search=$searchText&page=$page&per_page=$PER_PAGE&', passParameters: true, passToken: false));
  } else {
    it = await handleResponse(await buildHttpResponse('${APIEndPoint.productsList}?exclude=10267,10271&orderby=$orderBy&search=$searchText&page=$page&per_page=$PER_PAGE', passParameters: true, passToken: false));
  }
...
Future<List<CategoryModel>> getCategoryList() async {
  Iterable it = await handleResponse(await buildHttpResponse('${APIEndPoint.categories}?exclude=52,175', passParameters: true, passToken: false));
...
Future<CommonMessageResponse> getpolls({required int activiyid,required int bpoll}) async {
  Map request = {"activity_id": activiyid, "user_id": appStore.loginUserId,"bpolls_vote_optn":bpoll};
  return CommonMessageResponse.fromJson(
    await handleResponse(await buildHttpResponse('${APIEndPoint.bpolls}', method: HttpMethod.POST, request: request)),
  );
}
Future<List<dynamic>> membershipcheck() async {
  Map request = {"user_id": appStore.loginUserId,};
  return await handleResponse(await buildHttpResponse('${APIEndPoint.membershipcheck}', method: HttpMethod.GET, request: request)
  );
}
Future<List<dynamic>> membershipupdate(levelid) async {
  Map request = {"user_id":appStore.loginUserId,"level":levelid};

  return   await handleResponse(await buildHttpResponse('${APIEndPoint.membershipsupdate}', method: HttpMethod.POST, request: request));
}